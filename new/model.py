...
class EHR_LR(nn.Module):
    ...
    def __init__(self,input_size, embed_dim = 2):    
        ...
        #self.relu = nn.ReLU() relu not good, discard that 
    ...
    def forward(self, input):  #lets say the input the one sample data of the merged set  
        ...
        #embedded = torch.sum(embedded).view(1,-1)
        #output = embedded
        #for i in range(self.n_layers):
            #output, hidden = self.gru(output, hidden)
        #output = self.sigmoid(self.relu(embedded)) 
        #embedded = self.relu(embedded) #Non-linear activation, result not good discard 
