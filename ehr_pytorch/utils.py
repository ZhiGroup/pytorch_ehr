...
#import string
#import re
#import random   ##################### XIN: DO WE NEED TO IMPORT THESE??????????????????????????????????????????
import math 
import time 
import os
...
def trainbatches(loader, model, optimizer, shuffle = True, which_model = 'RNN'):#,we dont need this print print_every = 10, plot_every = 5): 
    ...
    for i,batch in enumerate(loader):
        ...
        if which_model == "LR":
            for pt in batch:
                out,loss =  trainsample(pt, model, optimizer, criterion = nn.BCELoss())
            output.append(out.cpu().data.numpy()[0][0])
        else:
...
def calculate_auc(model, loader, which_model = 'RNN', shuffle = True): # batch_size= 128 not needed
    ...
    for i,batch in enumerate(loader):
            ...
            if which_model != 'LR':
                ...
                # Minor case:LR Model
                for pt in batch:
                    out,label_tensor =  model(pt)
                    y_hat.append(out.cpu().data.numpy()[0][0])
