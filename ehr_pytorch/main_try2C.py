...
parser.add_argument('-L2', type=float, default=0, help='L2 regularization [default: 0.00001]')
parser.add_argument('-epochs', type=int, default=20, help='number of epochs for train [default: 20]')
parser.add_argument('-batch_size', type=int, default=200, help='batch size for training [default: 1]')
...
# model
parser.add_argument('-which_model', type = str, default = 'CNN', help='choose from {"LR", "RNN", "CNN"}')
parser.add_argument('-mb', type = bool, default =  False , help='whether train on mini batch (True) or not (False) [default: False ]') #at train
...
for ep in range(args.epochs):
    ...
    train_auc, y_real, y_hat = TrainVaTe.calculate_auc(model= ehr_model, data = train1, which_model = args.which_model, batch_size = args.batch_size)
    print ('\n Current running on: Epoch ', ep,' Training auc:', train_auc)
    TrainVaTe.auc_plot(y_real, y_hat)
    valid_auc, y_real, y_hat  = TrainVaTe.calculate_auc(model = ehr_model, data = valid1, which_model = args.which_model, batch_size = args.batch_size)
    print ('\n Current running on: Epoch ', ep,' validation auc:', valid_auc)
    TrainVaTe.auc_plot(y_real, y_hat)
    test_auc, y_real, y_hat = TrainVaTe.calculate_auc(model = ehr_model, data = test1, which_model = args.which_model, batch_size = args.batch_size)
